...
from _suite_paths import (  # noqa: E402
    REPO as PROJECT_ROOT, RUN_NAME, RUN_DIR as G1_RUN_DIR,
    ANALYSIS_OUT as ANALYSIS_DIR, CONFIG_YAML as DEFAULT_CONFIG,
)
...
def rewrite_summary_and_readme(out_dir: Path, args_text: str) -> None:
    ...
    data["script"] = "logs/milestone_g/reproducible_testing_suite/02_sampled_error_analysis.py"
    data["note"] = "Fresh sampled inference pass; not the exact original best-epoch validation sample."
...
def remove_unrequested_outputs(out_dir: Path) -> None:
    # group_feature_summary.csv is now KEPT: the RGB-shortcut fingerprint
    # (rgb_shortcut_fingerprint.py --preset g1) needs the per-outcome RGB/intensity
    # summary. It was previously deleted as "unrequested"; we retain it so the G1
    # bright-road fingerprint can run without a second inference pass.
    return
