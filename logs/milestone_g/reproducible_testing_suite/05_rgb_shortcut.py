...
from _suite_paths import REPO as REPO_ROOT  # noqa: E402
...
# Error-flow colours come from the shared palette (road->marking owns the suite's red).
GROUP_COLORS = dict(S.TRANSITION)
...
PRESETS = {
    ...
    "g1": {
        "run_dir": REPO_ROOT / "logs/milestone_g/run_analysis/G1_schedule_extend",
        "label": "G1",
    },
}
...
def plot_brightness_vs_intensity(fp: pd.DataFrame, label: str, out_path: Path) -> None:
    ...
    ax.axvline(float(fp.loc["road_tp", "brightness_mean"]), color=S.TRANSITION["road_tp"], linestyle=":", alpha=0.7)
    ax.axvline(float(fp.loc["marking_tp", "brightness_mean"]), color=S.TRANSITION["marking_tp"], linestyle=":", alpha=0.7)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--preset", choices=("f0", "g0", "g1", "f0_vs_g0", "g0_vs_g1"),
                   default=None, help="Convenience presets for repo default run dirs.")
...
def main() -> None:
    ...
    if args.preset == "f0":
        ...
        base_run_dir, base_label = PRESETS["g1"]["run_dir"], "G1"
    elif args.preset == "f0_vs_g0":
        base_run_dir, base_label = PRESETS["f0"]["run_dir"], "F0"
        cmp_run_dir, cmp_label = PRESETS["g0"]["run_dir"], "G0"
    elif args.preset == "g0_vs_g1":
        base_run_dir, base_label = PRESETS["g0"]["run_dir"], "G0"
        cmp_run_dir, cmp_label = PRESETS["g1"]["run_dir"], "G1"
