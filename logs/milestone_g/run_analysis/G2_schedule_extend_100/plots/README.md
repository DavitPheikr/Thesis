...
1. `loss_curves.png` - G2 train/validation total loss.
...
4. `lr_schedule.png` - LR changes; best epoch is 68.
...
7. `comparison_marking_curves.png` - G2 vs F0, D0 marking curves.
8. `predicted_true_marking_ratio_over_epochs.png` - marking over/underprediction.
9. `confusion_best_checkpoint.png` - row-normalized confusion at epoch 68.
10. `confusion_final_epoch.png` - row-normalized confusion at epoch 100.
11. `runtime_and_memory.png` - compute cost and memory.

Sampled best-checkpoint plots live under `sampled_error_analysis_epoch68/plots/`.
