...
OUT_DIR = PROJECT_ROOT / "logs/milestone_g/run_analysis/G1_schedule_extend"
...
def validate_artifacts(run_dir: Path, run_name: str, require_components: bool) -> dict[str, Any]:
    ...
    for key in ("eval_json_count", "confusion_count", "checkpoint_count"):
        if int(counts[key]) != expected:
            raise RuntimeError(f"{run_name}: {key}={counts[key]} but eval_history rows={expected}")
...
def validate_g1_config() -> dict[str, Any]:
    ...
    for key, expected_value in expected.items():
        ...
        if key == "class_weights":
            ...
            mismatches[key] = (actual, expected_value)
    if mismatches:
        raise RuntimeError(f"G1 config mismatch: {mismatches}")
    return checks
...
def write_visual_notes(path: Path) -> None:
    ...
    path.write_text(
        "\n".join(
            [
                "# G1 Visual Inspection Notes",
                "",
                "Use this file to record manual observations from front-camera overlay PNGs.",
                "",
                "Overlay location:",
                "`logs/milestone_g/run_analysis/front_camera_predictions/G1_schedule_extend/`",
                "",
                ...
                "- Does behavior change after the LR drop compared with G0?",
                "",
                "## Sequence Notes",
                "",
                "### 054",
...
def write_analysis_md(
    ...
    g1 = s("G1", "best")
    g1_final = s("G1", "final")
    g1_ratio = pred_true[(pred_true["run"] == "G1") & (pred_true["epoch"] == int(g1["epoch"])) & (pred_true["class"] == "marking")]["pred_true_ratio"].iloc[0]
    g0_ratio = pred_true[(pred_true["run"] == "G0") & (pred_true["epoch"] == int(g0["epoch"])) & (pred_true["class"] == "marking")]["pred_true_ratio"].iloc[0]
    ...
    lines = [
        ...
        "## Validation",
        "",
        f"- feature mode: `{config_checks['feature_mode']}`",
        f"- loss: `{config_checks['loss_name']}` with lambda `{config_checks['lovasz_lambda']}`",
        f"- scheduler patience: `{config_checks['scheduler_patience']}`",
        f"- loss composition residual, val: `{loss_check['val_total_vs_components']:.2e}`",
        "",
        "## Headline",
        "",
        "| run | epoch | marking IoU | F1 | precision | recall | mIoU | pred/true marking | LR |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        f"| G0 best | {int(g0['epoch'])} | {g0['marking_iou']:.6f} | {g0['marking_f1']:.6f} | {g0['marking_precision']:.6f} | {g0['marking_recall']:.6f} | {g0['miou']:.6f} | {g0_ratio:.3f} | {g0['lr']:.6f} |",
        f"| G1 best | {int(g1['epoch'])} | {g1['marking_iou']:.6f} | {g1['marking_f1']:.6f} | {g1['marking_precision']:.6f} | {g1['marking_recall']:.6f} | {g1['miou']:.6f} | {g1_ratio:.3f} | {g1['lr']:.6f} |",
        f"| G1 final | {int(g1_final['epoch'])} | {g1_final['marking_iou']:.6f} | {g1_final['marking_f1']:.6f} | {g1_final['marking_precision']:.6f} | {g1_final['marking_recall']:.6f} | {g1_final['miou']:.6f} | | {g1_final['lr']:.6f} |",
        "",
        "## LR Events",
        "",
    ]
...
def main() -> None:
    ...
    artifact_counts = pd.DataFrame(
        [
            validate_artifacts(D0_RUN_DIR, "D0", False),
            validate_artifacts(F0_RUN_DIR, "F0", False),
            validate_artifacts(G0_RUN_DIR, "G0", True),
            validate_artifacts(G1_RUN_DIR, "G1", True),
        ]
    )
    ...
    histories = {
        "D0": load_history(D0_RUN_DIR),
        "F0": load_history(F0_RUN_DIR),
        "G0": load_history(G0_RUN_DIR),
        "G1": load_history(G1_RUN_DIR),
    }
    components = {
        "G0": load_components(G0_RUN_DIR),
        "G1": load_components(G1_RUN_DIR),
    }
    validate_loss_components(G0_RUN_DIR, histories["G0"])
    g1_loss_check = validate_loss_components(G1_RUN_DIR, histories["G1"])

    summary_rows = [
        row_metrics("D0", "official", histories["D0"][histories["D0"]["epoch"] == D0_OFFICIAL_EPOCH].iloc[0], None),
        row_metrics("F0", "best", best_row(histories["F0"]), None),
        row_metrics("F0", "final", histories["F0"].iloc[-1], None),
        row_metrics("G0", "best", best_row(histories["G0"]), components["G0"]),
        row_metrics("G0", "final", histories["G0"].iloc[-1], components["G0"]),
        row_metrics("G1", "best", best_row(histories["G1"]), components["G1"]),
        row_metrics("G1", "final", histories["G1"].iloc[-1], components["G1"]),
    ]
    summary = pd.DataFrame(summary_rows)
    summary.to_csv(OUT_DIR / "summary.csv", index=False)

    g1_best_epoch = int(summary[(summary["run"] == "G1") & (summary["label"] == "best")]["epoch"].iloc[0])
    if not (G1_RUN_DIR / "checkpoints" / f"ckpt_epoch_{g1_best_epoch:05d}.pth").exists():
        raise FileNotFoundError(f"G1 best checkpoint missing for epoch {g1_best_epoch}")
    ...
    for run in ("G0", "G1"):
        ...
        comp = components[run].copy()
        merged = hist.merge(comp, on="epoch", how="left")
        merged.insert(0, "run", run)
        epoch_rows.append(merged)
    pd.concat(epoch_rows, ignore_index=True).to_csv(OUT_DIR / "g0_vs_g1_epoch_metrics.csv", index=False)

    pred_true = pd.concat(
        [all_epoch_pred_true("G0", G0_RUN_DIR), all_epoch_pred_true("G1", G1_RUN_DIR)],
        ignore_index=True,
    )
    pred_true.to_csv(OUT_DIR / "pred_true_ratio_by_epoch.csv", index=False)

    events = pd.concat([lr_events("G0", histories["G0"]), lr_events("G1", histories["G1"])], ignore_index=True)
    events.to_csv(OUT_DIR / "lr_events.csv", index=False)

    confusion = []
    for run, run_dir in (("G0", G0_RUN_DIR), ("G1", G1_RUN_DIR)):
        for label in ("best", "final"):
            epoch = int(summary[(summary["run"] == run) & (summary["label"] == label)]["epoch"].iloc[0])
            confusion.extend(confusion_rows(run, label, epoch, load_confusion(run_dir, epoch)))
    pd.DataFrame(confusion).to_csv(OUT_DIR / "confusion_breakdown.csv", index=False)

    loss_summary = loss_component_summary(histories["G1"], components["G1"])
    loss_summary.to_csv(OUT_DIR / "loss_component_summary.csv", index=False)
    align = loss_alignment(loss_summary)
    align.to_csv(OUT_DIR / "loss_alignment.csv", index=False)

    write_visual_notes(OUT_DIR / "visual_inspection_notes.md")
    write_analysis_md(summary, best_vs_final, pred_true, events[events["run"] == "G1"], config_checks, g1_loss_check)

    print(f"g1_best_epoch {g1_best_epoch}")
