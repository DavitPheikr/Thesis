#!/usr/bin/env python
"""Generate the exact G1 schedule-extension plot package and conclusions."""
...
G0_RUN_DIR = PROJECT_ROOT / "logs/milestone_g/runs/G0_rgb_lovasz"
OUT_DIR = PROJECT_ROOT / f"logs/milestone_g/run_analysis/{RUN_NAME}"
...
def style(ax: plt.Axes) -> None:
    ax.grid(True, axis="y", color=GRID_COLOR, alpha=0.45)
...
def load_inputs() -> dict[str, pd.DataFrame]:
    files = {
        ...
        "epoch": OUT_DIR / "g0_vs_g1_epoch_metrics.csv",
...
def g1_best_epoch(summary: pd.DataFrame) -> int:
    return int(summary[(summary["run"] == "G1") & (summary["label"] == "best")]["epoch"].iloc[0])


def g1_final_epoch(summary: pd.DataFrame) -> int:
    return int(summary[(summary["run"] == "G1") & (summary["label"] == "final")]["epoch"].iloc[0])
...
def plot_loss_curves(data: dict[str, pd.DataFrame]) -> None:
    g1 = data["epoch"][data["epoch"]["run"] == "G1"]
    ...
    ax.set_title("G1 train and validation total loss")
    style(ax)
    ax.legend()
...
def plot_loss_components(data: dict[str, pd.DataFrame], split: str, out_name: str) -> None:
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("loss")
    ax.set_title(f"G1 {split} loss components")
    style(ax)
    ax.legend()
...
def plot_lr_schedule(data: dict[str, pd.DataFrame]) -> None:
    g1 = data["epoch"][data["epoch"]["run"] == "G1"]
    events = data["events"][data["events"]["run"] == "G1"]
    best = g1_best_epoch(data["summary"])
    fig, ax = plt.subplots(figsize=(10.8, 5.8))
    ...
    ax.set_title("G1 ReduceLROnPlateau schedule")
...
def plot_marking_metrics(data: dict[str, pd.DataFrame]) -> None:
    ...
    for col, label, color in [
        ("lane_iou", "IoU", MARKING_COLOR),
        ("lane_precision", "precision", ROAD_COLOR),
        ("lane_recall", "recall", OTHER_COLOR),
        ("lane_f1", "F1", G1_COLOR),
    ]:
    ...
    ax.set_ylabel("score")
    ax.set_ylim(0, 1.0)
    ax.set_title("G1 marking metrics over epochs")
    style(ax)
    ax.legend(ncol=2)
...
def plot_iou_per_class(data: dict[str, pd.DataFrame]) -> None:
    ...
    ax.set_ylabel("IoU")
    ax.set_ylim(0, 1.0)
    ax.set_title("G1 per-class IoU and mIoU over epochs")
    style(ax)
    ax.legend(ncol=2)
    savefig(fig, PLOTS_DIR / "iou_per_class_and_miou_over_epochs.png")


def plot_g0_vs_g1(data: dict[str, pd.DataFrame]) -> None:
    epoch = data["epoch"]
    g0 = epoch[epoch["run"] == "G0"]
    g1 = epoch[epoch["run"] == "G1"]
    panels = [
    ...
    for ax, (col, label) in zip(axes.ravel(), panels):
        ax.plot(g0["epoch"], g0[col], marker="o", linewidth=1.9, label="G0", color=G0_COLOR)
        ax.plot(g1["epoch"], g1[col], marker="o", linewidth=2.2, label="G1", color=G1_COLOR)
        ax.set_xlabel("epoch")
        ax.set_ylabel(label)
        ax.set_title(label)
        style(ax)
        ax.legend()
    fig.suptitle("G0 vs G1 marking curves", y=0.995, fontsize=15)
    savefig(fig, PLOTS_DIR / "g0_vs_g1_marking_curves.png")
...
def plot_pred_true(data: dict[str, pd.DataFrame]) -> None:
    ...
    best = g1_best_epoch(data["summary"])
    fig, ax = plt.subplots(figsize=(11.2, 6.0))
    for run, color in [("G0", G0_COLOR), ("G1", G1_COLOR)]:
        sub = df[(df["run"] == run) & (df["class"] == "marking")].sort_values("epoch")
        ax.plot(sub["epoch"], sub["pred_true_ratio"], marker="o", linewidth=2.1, label=run, color=color)
    ax.axhline(1.0, color="black", linestyle=":", linewidth=1.3, label="perfect ratio")
    ax.axvline(best, color="black", linestyle="--", linewidth=1.0, alpha=0.6)
    ax.set_xlabel("epoch")
...
def confusion_matrix(epoch: int) -> np.ndarray:
    ...
    if cm.shape != (3, 3):
        raise RuntimeError(f"G1 confusion epoch {epoch} shape {cm.shape}; expected (3, 3)")
...
def plot_runtime(data: dict[str, pd.DataFrame]) -> None:
    ...
    axes[0].set_title("G1 runtime per epoch")
    style(axes[0])
    ...
    axes[1].set_title("G1 validation time and memory")
    style(axes[1])
...
def plot_sampled_distance(data: dict[str, pd.DataFrame]) -> None:
    ...
    for col, label, color in [
        ...
        ("marking_precision", "precision", ROAD_COLOR),
        ("marking_recall", "recall", OTHER_COLOR),
        ("marking_f1", "F1", G1_COLOR),
    ]:
        axes[0].plot(x, df[col], marker="o", linewidth=2.0, label=label, color=color)
    axes[0].set_ylabel("score")
    axes[0].set_ylim(0, 1)
    axes[0].set_title("G1 best-checkpoint marking metrics by distance")
    ...
    axes[1].set_ylabel("pred/true marking")
    axes[1].set_xlabel("distance bucket")
    axes[1].set_xticks(x, labels)
    axes[1].set_title("G1 best-checkpoint marking calibration by distance")
    style(axes[1])
...
def plot_sampled_rgb_valid(data: dict[str, pd.DataFrame]) -> None:
    ...
    width = 0.25
    axes[0].bar(x - width, sub["active_point_share"], width, label="active point share", color="#999999")
    axes[0].bar(x, sub["true_marking_share"], width, label="true marking share", color=MARKING_COLOR, alpha=0.75)
    axes[0].bar(x + width, sub["predicted_marking_share"], width, label="predicted marking share", color=G1_COLOR, alpha=0.75)
    axes[0].set_ylabel("share")
    axes[0].set_title("G1 sampled point contribution by RGB validity")
    axes[0].legend()
    style(axes[0])
    for col, label, color in [
        ("marking_iou", "IoU", MARKING_COLOR),
        ("marking_precision", "precision", ROAD_COLOR),
        ("marking_recall", "recall", OTHER_COLOR),
        ("predicted_true_marking_ratio", "pred/true", "black"),
    ]:
        axes[1].plot(x, sub[col], marker="o", linewidth=2.0, label=label, color=color)
    axes[1].axhline(1.0, color="black", linestyle=":", linewidth=1.0, alpha=0.6)
    axes[1].set_xticks(x, order)
    axes[1].set_ylabel("score / ratio")
    axes[1].set_title("G1 best-checkpoint metrics by RGB validity")
...
def write_plots_readme(data: dict[str, pd.DataFrame]) -> None:
    ...
    text = f"""# G1 Plot Package

Generated by `plot_g1_schedule_extend_analysis.py`.

This folder intentionally contains only the requested 11 core plots.

1. `loss_curves.png` - G1 train/validation total loss.
...
7. `g0_vs_g1_marking_curves.png` - direct G0 vs G1 marking curves.
...
def write_conclusions(data: dict[str, pd.DataFrame]) -> None:
    ...
    g0 = summary[(summary["run"] == "G0") & (summary["label"] == "best")].iloc[0]
    g1 = summary[(summary["run"] == "G1") & (summary["label"] == "best")].iloc[0]
    g1_final = summary[(summary["run"] == "G1") & (summary["label"] == "final")].iloc[0]
    pred = data["pred_true"]
    g0_pt = pred[(pred["run"] == "G0") & (pred["epoch"] == int(g0["epoch"])) & (pred["class"] == "marking")]["pred_true_ratio"].iloc[0]
    g1_pt = pred[(pred["run"] == "G1") & (pred["epoch"] == int(g1["epoch"])) & (pred["class"] == "marking")]["pred_true_ratio"].iloc[0]
    ...
    lines = [
        ...
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        f"| G0 best | {int(g0['epoch'])} | {g0['marking_iou']:.6f} | {g0['marking_f1']:.6f} | {g0['marking_precision']:.6f} | {g0['marking_recall']:.6f} | {g0['miou']:.6f} | {g0_pt:.3f} |",
        f"| G1 best | {int(g1['epoch'])} | {g1['marking_iou']:.6f} | {g1['marking_f1']:.6f} | {g1['marking_precision']:.6f} | {g1['marking_recall']:.6f} | {g1['miou']:.6f} | {g1_pt:.3f} |",
        f"| G1 final | {int(g1_final['epoch'])} | {g1_final['marking_iou']:.6f} | {g1_final['marking_f1']:.6f} | {g1_final['marking_precision']:.6f} | {g1_final['marking_recall']:.6f} | {g1_final['miou']:.6f} | |",
        "",
        "## Interpretation",
        "",
        f"- G1 best marking IoU changed by `{g1['marking_iou'] - g0['marking_iou']:+.6f}` versus G0 best.",
        f"- G1 best precision changed by `{g1['marking_precision'] - g0['marking_precision']:+.6f}`.",
        f"- G1 best recall changed by `{g1['marking_recall'] - g0['marking_recall']:+.6f}`.",
        f"- G1 final-minus-best IoU drift is `{g1_final['marking_iou'] - g1['marking_iou']:+.6f}`.",
        "",
    ...
    (OUT_DIR / "g1_conclusions.md").write_text("\n".join(lines))
...
def main() -> None:
    ...
    plot_g0_vs_g1(data)
    plot_pred_true(data)
    plot_confusion(best, "confusion_best_checkpoint.png", f"G1 epoch {best} confusion matrix")
    plot_confusion(final, "confusion_final_epoch.png", f"G1 epoch {final} confusion matrix")
    plot_runtime(data)
