#!/usr/bin/env python
"""Sampled E0 epoch-14 error analysis with RGB-valid stratification.

This runs a fresh sampled validation inference pass for the E0 best checkpoint.
It is not the exact epoch-14 validation sample from training. The purpose is to
answer the key E0 diagnostic question:

    Are E0 false-positive markings concentrated where RGB is valid?

Outputs are written under:

    logs/milestone_e/run_analysis/E0_rgb_front_v1/sampled_error_analysis_epoch14/
"""
...
def find_project_root() -> Path:
    for parent in Path(__file__).resolve().parents:
        if (parent / "logs/milestone_e/configs/e0_rgb_front.yml").exists():
            return parent
...
RUN_NAME = "E0_rgb_front_v1"
DEFAULT_CONFIG = PROJECT_ROOT / "logs/milestone_e/configs/e0_rgb_front.yml"
DEFAULT_CHECKPOINT = (
    PROJECT_ROOT / f"logs/milestone_e/runs/{RUN_NAME}/checkpoints/ckpt_epoch_00014.pth"
)
DEFAULT_OUT_DIR = (
    PROJECT_ROOT / f"logs/milestone_e/run_analysis/{RUN_NAME}/sampled_error_analysis_epoch14"
)
...
def load_cfg(path: Path, steps: int) -> dict[str, Any]:
    ...
    dataset_cfg["cache_dir"] = repo_path(
        dataset_cfg.get("cache_dir"), "logs/milestone_e/cache/E0_rgb_front_v1"
    )
    dataset_cfg["test_result_folder"] = repo_path(
        dataset_cfg.get("test_result_folder"), "logs/milestone_e/test_results/E0_rgb_front_analysis"
    )
...
class E0DatasetWithRaw(PandaSetFFLane3Dataset):
    """Analysis-only E0 dataset that returns raw labels alongside RGB features."""
...
def attach_analysis_preprocess_and_transform(model) -> None:  # noqa: ANN001
    ...
    def transform_with_raw(data, attr, min_possibility_idx=None):  # noqa: ANN001, ARG001
        ...
        if attr["split"] in ["training", "train"]:
            raise RuntimeError("E0 sampled error analysis must not run on training split.")
...
def main() -> None:
    ...
    dataset = E0DatasetWithRaw(**cfg["dataset"])
    ...
    with torch.no_grad():
        for step, inputs in enumerate(tqdm(loader, desc="e0_sampled_validation"), start=1):
    ...
    summary = {
        ...
        "note": "Fresh sampled inference pass; not exact original E0 epoch-14 validation sample.",
    ...
    print("E0 sampled error analysis complete")
