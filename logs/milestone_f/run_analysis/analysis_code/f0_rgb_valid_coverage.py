#!/usr/bin/env python
"""Milestone F0 RGB-valid coverage analysis for the validation split.

This script does not run model inference. It loads the F0 RGB-front dataset
feature path and summarizes where RGB is valid/invalid in the validation split.

Outputs:
    logs/milestone_f/run_analysis/F0_rgb_soft_weights/frame_rgb_valid_stats.csv
    logs/milestone_f/run_analysis/F0_rgb_soft_weights/per_sequence_rgb_valid_metrics.csv
    logs/milestone_f/run_analysis/F0_rgb_soft_weights/rgb_valid_stratification_summary.md
"""
...
def main() -> None:
    ...
    print("=" * 80)
    print("MILESTONE F0 RGB-VALID VALIDATION COVERAGE")
    print("=" * 80)
    print(
    ...
    dataset = PandaSetFFLane3Dataset(
        dataset_path=None,
        cache_dir=str(REPO_ROOT / "logs/milestone_f/cache/F0_rgb_soft_weights_v1"),
        use_cache=False,
    ...
    for idx in range(len(split)):
        ...
        if feat.ndim != 2 or feat.shape[1] != 5:
            raise ValueError(
                f"Expected F0 dataset feature shape (N, 5), got {feat.shape} for {attr}"
            )
    ...
    lines = [
        "# F0 RGB-Valid Coverage Summary",
        "",
        "This is a dataset coverage diagnostic, not a checkpoint inference report.",
        "It tells us where F0 had front-camera RGB available in the validation split.",
        "",
    ...
    lines.extend(
        [
            ...
            "whether F0 false positives occur mostly where `rgb_valid=1` or",
            "`rgb_valid=0`. That requires the epoch-13 sampled inference analysis.",
            "",
        ]
    )
