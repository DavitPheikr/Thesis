...
def _router_jit(normal_x, normal_y, v1, v2, inc_x, inc_y):
    ...
    if sin2_t2 > 1.0:
        return (inc_ux + 2.0 * cos_t1 * nrm_ux, inc_uy + 2.0 * cos_t1 * nrm_uy)
    ...
    return (par_x + perp_scale * perp_dx, par_y + perp_scale * perp_dy)
...
@njit(cache=True)
def _update_pheromones_jit(
    tau, iter_tour, iter_cost, best_tour, best_cost, rho, Q, has_best
):
...
class ACO:
    ...
    def _build_tour(self):
        ...
        for _ in range(n - 1):
            ...
            total = _roulette_total(visited, tau_row, eta_row, self.alpha, self.beta)
            ...
            if total < 1e-15:
                ...
                r = random.random()
                chosen = _roulette_pick(
                    visited, tau_row, eta_row, self.alpha, self.beta, r * total
                )
                if chosen < 0:
...
def load_tsp(file_path, use_file_velocity=False):
    ...
    if is_tsplib:
        ...
        if len(city) != n:
            raise ValueError(
                f"TSPLIB parse error: expected {n} cities, got {len(city)}"
            )
...
def save_results(
    ...
    ax2.axhline(
        pwa_cost, color="coral", linestyle="--", label=f"PWA best ({pwa_cost:.4f})"
    )
    ...
    metrics = {
        ...
        "n_ants": int(n_ants),
        "n_iters": int(n_iters),
        "pwa_cost": float(pwa_cost) if pwa_cost != float("inf") else None,
        "hybrid_cost": float(final_cost),
        "improvement_pct": float(improvement),
        "elapsed_seconds": float(elapsed),
        "peak_memory_mb": float(peak_mem_mb),
        "best_tour": [int(x) for x in best_tour],
        "history": [float(x) for x in aco_history],
    }
...
def main():
    ...
    parser.add_argument(
        "--velocity", "--veloctity", dest="use_file_velocity", action="store_true"
    )
    ...
    parser.add_argument(
        "--seed", type=int, default=42, help="RNG seed for ACO ants (default: 42)"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="experiments",
        dest="output_dir",
        help="Directory for --save-results output (default: experiments)",
    )
    ...
    filename, n, city, cost, velocity = load_tsp(
        args.input_file, args.use_file_velocity
    )
    ...
    if pwa_tours:
        print(
            f"  Warm-starting pheromone from {args.elite_pct:.0f}% elite PWA tours "
    ...
    improvement = (
        (pwa_cost - best_cost) / pwa_cost * 100 if pwa_cost < float("inf") else 0.0
    )
    ...
    if args.save_results:
        save_results(
            ...
            seed=args.seed,
            use_velocity=args.use_file_velocity,
            n_ants=args.ants,
            n_iters=args.iters,
